...
annotate_maf <- function(maf, fillout, normal.count=5) {
...
parse_fillout_maf <- function(fillout) {
    ...
    fillout = fillout[!duplicated(fillout$tmp_id),]
    # Calculate frequencies and return
    return(group_by(fillout, TAG) %>% summarize(normal_count = sum(t_alt_count>=3)))
}
...
if( ! interactive() ) {
    ...
    parser$add_argument('-n', '--normal_count', type='double', default=5, help='Minimum number of normal panel samples with 3+ supporting reads')
